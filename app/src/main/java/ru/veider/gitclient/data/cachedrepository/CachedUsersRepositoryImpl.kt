    ...
    override fun getUsers(since:Long): Single<List<UserEntity>> {
        return remoteRepository.getUsers(since)
            .doOnSuccess { userList ->
                handler.post {
                    Toast.makeText(appContext, appContext.getString(R.string.web_data), Toast.LENGTH_SHORT).show()
                }
            ...
            .onErrorResumeWith {
                handler.post {
                    Toast.makeText(appContext, appContext.getString(R.string.local_data), Toast.LENGTH_SHORT).show()
                }
